...
The following code scales moveability to population density, but that doesn't
help here.
```{r, eval = FALSE}
library (moveability)
data_dir <- "/data/data/moveability/accra"
...
```
then repeat the above code to calculating moveability using these `attractors`.
Also scale moveability to the sqrt of numbers of activity centres.
```{r, eval = FALSE}
